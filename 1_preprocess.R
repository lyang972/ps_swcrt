# 1_process.R
# Preprocessing: assemble long-format data, build indices, and prepare Stan data list.

stopifnot(exists("CONFIG_PATHS"), exists("data_path"))
...
stopifnot(all(column_question %in% colnames(tb)))

data <- tb[, column_question]; names(data) <- column_name
# Harmonize orientation category
...
df <- df %>%
  dplyr::mutate(
    ...
    time = time_idx_R + 1,
    # Mediator M (sum of 6 items + reverse-coded item 7)
    ...
    # Outcome Y (combine facility- and self-testing after baseline)
...
# Stepped-wedge design: cluster start time and province
...
df <- df %>%
  ...
  dplyr::group_by(ID) %>% dplyr::arrange(time) %>%
  dplyr::mutate(Duration = cumsum(Z)) %>% dplyr::ungroup()
...
Individual_to_Cluster_Map <- data_model %>% dplyr::select(i_idx, j_idx, Province) %>% dplyr::distinct() %>% dplyr::arrange(i_idx)
Provinces_List <- sort(unique(Individual_to_Cluster_Map$Province))

# X_ij (individual covariates)
X_data <- data_model %>% dplyr::select(i_idx, Age_centered, Marital, Education, Income, Orientation) %>% dplyr::distinct() %>% dplyr::arrange(i_idx)
X_matrix_full <- model.matrix(~ Age_centered + Marital + Education + Income + Orientation, data = X_data)
X_matrix <- if (ncol(X_matrix_full) > 1) X_matrix_full[, -1, drop = FALSE] else matrix(0, nrow(X_data), 0)
K_X <- ncol(X_matrix)

# C_j (cluster covariates)
C_data <- data_model %>% dplyr::select(j_idx, Province) %>% dplyr::distinct() %>% dplyr::arrange(j_idx)
C_matrix_full <- model.matrix(~ Province, data = C_data)
C_matrix <- if (ncol(C_matrix_full) > 1) C_matrix_full[, -1, drop = FALSE] else matrix(0, nrow(C_data), 0)
K_C <- ncol(C_matrix)
